...
class AdRanker {
    ...
    static List<Ad> rank(List<Ad> ads, SuccessRateTracker successRateTracker, int lives) {
        return ads.stream()
                .sorted(Comparator
                        .comparingDouble((Ad ad) -> -successRateTracker.successRate(ProbabilityTier.fromLabel(ad.probability())))
                        .thenComparingInt(ad -> -ad.reward()))
                .toList();
    }
    ...
    static double riskAdjustedScore(Ad ad, SuccessRateTracker successRateTracker, int lives) {
        ...
        return ad.reward() * successProbability - lifeCost * (1 - successProbability);
    }
}
